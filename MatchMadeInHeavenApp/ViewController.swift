...
class ViewController: UIViewController, UITextFieldDelegate{
    ...
    override func viewDidLoad() {
        ...
        firstDatePicker?.addTarget(self, action: #selector(dateChanged(datePicker:)), for: .valueChanged)
        firstDatePicker!.date = Calendar.current.date(byAdding: .year, value: -21, to: Date())!
        secondDatePicker = UIDatePicker()
        secondDatePicker?.datePickerMode = .date
        secondDatePicker?.addTarget(self, action: #selector(dateChanged(datePicker:)), for: .valueChanged)
        secondDatePicker!.date = Calendar.current.date(byAdding: .year, value: -21, to: Date())!
    
        firstPersonDOB.text = dateFormatter.string(from: firstDatePicker!.date)
        secondPersonDOB.text = dateFormatter.string(from:secondDatePicker!.date)
        firstPersonDOB.inputView = firstDatePicker
        secondPersonDOB.inputView = secondDatePicker
    ...
    func validation(){
        if( firstPersonLocation.text != "" && secondPersonLocation.text != "" && (Int(personYear[0])! >= 1900 && Int(personYear[0])! <= 2018) && (Int(personYear[1])! >= 1900 && Int(personYear[1])! <= 2018)){
